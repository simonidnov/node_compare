  ...
  checkIt : function(){
    ...
    index.sdk.api.get('/coupon_code/valid', {code:$('#coupon_code').val().toUpperCase()}, function(e){
      ...
      if(e.datas.length === 0){
        ...
        var has_valid = false;
        $('.coupon_datas').append('<div class="message">Ce code est valide pour télécharger :</div><ul class="coupon_list"></ul>');
        for(var i=0; i<e.datas.length; i++){
          if(e.datas[i].offer === "chansonspersonnalisees"){
            ...
            if($('[data-offer="'+e.datas[i].offer+'"]').length === 0){
              $('.coupon_list').append('<li class=""><b>'+e.datas[i].label+'</b><br>'+e.datas[i].description+'<br><div class="btn btn-success buyWithCoupon" data-offer="'+e.datas[i].offer+'" data-productid="'+e.datas[i].product_id+'"  data-couponcode="'+e.datas[i].code+'"  data-couponid="'+e.datas[i]._id+'" data-amount="'+e.datas[i].amount+'">Télécharger '+e.datas[i].label+'</a></li>');
            }
          }
