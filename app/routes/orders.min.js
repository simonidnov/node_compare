...
"/public/stylesheets/ui.css","/public/stylesheets/components/formular.css"]});b.end()})}).post("/refund",function(a,b,c){Auth_helper.validate_admin(a,function(a){200===a.status?c():b.status(a.status).send(a)})},function(a,b,c){Orders_controller.refundCharge(a,b,function(a){b.status(a.status).send(a);b.end()})}).post("/",function(a,b,c){Orders_controller.create(a.body.data,b,function(a){200===a.status?b.status(a.status).send({status:a.status,response_display:{title:"Commande",message:"Votre commande est pr\u00eate \u00e0 \u00eatre r\u00e9gl\u00e9e.",
buttons:["OK"]}}):b.status(a.status).send(a)})}).put("/",function(a,b,c){Orders_controller.update(a.body.data,b,function(a){b.status(a.status).send({status:a.status,infos:a.datas})})})["delete"]("/",function(a,b,c){Orders_controller["delete"](a,b,function(a){b.status(a.status).send({status:a.status,infos:a.datas})})}).post("/buy_product_with_coupon_code",function(a,b,c){Orders_controller.buy_with_coupon(a,b,function(a){b.status(a.status).send(a)})});module.exports=orders;
