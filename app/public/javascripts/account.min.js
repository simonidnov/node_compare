...
$(".wallet_infos .message").append("En validant, vous utiliserez les coupons suivants :<br><b>"+b+"</b>");$(".wallet_infos").addClass("showed")})},create_forms:function(){this.checkout_form=new formular("#checkout_form",function(a){"blur"===a.status&&3<=$("#coupon_code").val().length&&index.sdk.api.get("/coupon_code/valid",{code:$("#coupon_code").val()},function(a){1<=a.datas.length?(0===$('[data-code="'+a.datas[0].code+'"]').length&&$("#coupons_list").append('<li data-code="'+a.datas[0].code+'" data-amount="'+
a.datas[0].amount+'"><div class="code">coupon code : '+a.datas[0].code+'</div><div class="amount">'+a.datas[0].label+" : "+a.datas[0].amount/100+" \u20ac</div></li>"),$("#coupon_code").val("").parent().removeClass("notempty").removeClass("valid"),account.coupons.push(a.datas[0]),account.check_amount(),$("#coupon_code").parent().find(".input_error").remove()):$("#coupon_code").parent().removeClass("valid").addClass("invalid").append('<div class="input_error">'+$("#coupon_code").attr("data-errormessage")+
"</div>")});"hitted"===a.status&&"submit"===a.action&&index.sdk.api.post("/orders/transaction",{basket_id:$("#order_transaction").attr("data-basketid"),coupons:account.coupons},function(){})});this.checkout_form.init();this.public_form=new formular("#public_datas",function(a){"hitted"===a.status&&"submit"===a.action&&(a=account.public_form.get_datas(),index.sdk.api.put("/account/profile/",a,function(a){}))});this.public_form.init();this.kid_form=(new formular("#add_kid",function(a){if("hitted"===
a.status&&"submit"===a.action){var b={};$.each($("#add_kid form").serializeArray(),function(a,d){b[d.name]=d.value});index.sdk.api.post("/me/members",b,function(a){window.location.reload()})}})).init();this.security_form=(new formular("#security_form",function(a){"hitted"===a.status&&"submit"===a.action&&index.sdk.api.put("/api/change_password/",{user_id:$("#security_form #user_id").val(),password:$("#security_form #change_password_old").val(),new_password:$("#security_form #change_password_new").val(),
retype_new_password:$("#security_form #retype_change_password_new").val()},function(a){})})).init();this.address_form=(new formular("#address_form",function(a){"validated"===a.status&&($("#addressLine1").val(),$("#postalCode").val(),$("#city").val(),$("#addressCountry").val())})).init();this.private_form=(new formular("#private_datas",function(a){})).init();this.services_form=new formular("#services_form",function(a){"hitted"===a.status&&"submit"===a.action&&(a=account.services_form.get_datas(),index.sdk.api.put("/account/profile/",
...
"/auth/device",{uid:$(this).attr("data-device_uid")},function(b){a.parent().parent().remove()})})}};
