...
sms_services:{type:"Object"},notifications:{type:"Boolean"},notifications_services:{type:"Object"},rights:{type:"Object"},validated:{type:"Boolean"},created:{type:"Date","default":Date.now},updated:{type:"Date","default":Date.now},public_locale:{type:"Boolean"},public_profile:{type:"Boolean"},public_kids:{type:"Boolean"},public_created:{type:"Boolean"},public_gamification:{type:"Boolean"},public_interest:{type:"Boolean"}},machineId=require("node-machine-id"),ua_parser=require("ua-parser-js"),device_uid=
machineId.machineIdSync({original:!0});0===db.connection.readyState&&db.connect(config.database.users,{useMongoClient:!0});var userSchemas=new db.Schema(user_datas),User=db.model("User",userSchemas);db.connection.on("open",function(a){});db.connection.on("error",function(a){});User.collection.dropIndexes();module.exports={attributes:user_datas};module.exports.getCount=function(){return User.find({}).count()};
...
module.exports.getFullUser=function(a,b){User.findOne({_id:a},function(c,d){c?b({status:401,message:"UNAUTHORISED_TOKEN",datas:c}):(console.log("GET FULL USER ",d),null!==d?Members_model.get(a,null,function(a){d.members=a.datas;b({status:200,message:"success me",datas:d})}):b({status:404,message:"USER_NOT_FOUND",datas:d}))})};
...
message:"USER_VALIDATED",datas:f})})})})};module.exports.getUsersDevice=function(a,b){User.find({devices:{$elemMatch:{uid:a}}},{email:1,avatar:1,pseudo:1},function(a,d){a||0===d.length?b({status:304,message:"NEW_DEVICE",datas:a}):b({status:200,message:"liste des utilisateurs ayant utilis\u00e9 ce device",users_device:d})})};
