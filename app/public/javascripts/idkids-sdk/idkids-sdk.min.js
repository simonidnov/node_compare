...
this.api.parent=this;this.api.options=g;this.init=function(a){this.check_cookies();window.matchMedia("(display-mode: standalone)");"undefined"===typeof Unity&&(Unity={call:function(a){var b=document.createElement("IFRAME");b.setAttribute("src","unity:"+a);document.documentElement.appendChild(b);document.documentElement.removeChild(b)}});this.api.get_device_uid(function(a){});this.api.reset_user(function(a){});this.api.get("/me/from",{secret:{}},function(b){a(b)});var c=new URL(window.location.href);
...
c,b,null,function(a){b(a)})};this.template=function(a,c,b){$.get(this.api.config.url+"/templating/"+a,c,function(a){b(a)})};this.createAuthbutton=function(a,c){null===document.getElementById(a)?c({status:"error",message:"TARGET_ID_TAG_ELEMENT_NOT_FOUND"}):this.template("auth_button",{user:this.api.user,options:this.options},function(b){document.getElementById(a).innerHTML=b;c({status:"success",message:"AUTH_BUTTON_CREATED"})})};this.createNotificationButton=function(a,c){null===document.getElementById(a)?
c({status:"error",message:"TARGET_ID_TAG_ELEMENT_NOT_FOUND"}):this.template("notif_button",{user:this.api.user,options:this.options},function(b){document.getElementById(a).innerHTML=b;c({status:"success",message:"AUTH_BUTTON_CREATED"})})};this.validInitedApp=function(a){this.api.get(this.api.config.url+"/api/apps/validate",{secret:this.options.secret},function(c){a(c)})};this.reset_listeners=function(){var a=this;$("[data-idkidssdk]").off("click").on("click",function(c){switch($(this).attr("data-idkidssdk")){case "add_basket":a.api.post("/basket",
{product_id:$(this).attr("data-productid"),quantity:$(this).attr("data-quantity"),url:window.location.href},function(b){200===b.status&&(b=parseInt($("#notifications .indice").html())+1,$("#notifications .indice").html(b),$("#total_basket").html("("+b+")"));a.callback({status:"ADD_BASKET"})});break;case "delete_basket_product":a.api.deleting("/basket",{basket_id:$(this).attr("data-basketid"),product_id:$(this).attr("data-productid")},function(a){200===a.status&&window.location.reload()});break;default:console.log("action not reconized by idkids sdk, please upgrade your sdk version")}})};
