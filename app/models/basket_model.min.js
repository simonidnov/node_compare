...
"default":Date.now},updated:{type:"Date","default":Date.now}};0===db.connection.readyState&&db.connect(config.database.users,{useMongoClient:!0});var basketSchemas=new db.Schema(basket_datas),Baskets=db.model("Baskets",basketSchemas);module.exports={attributes:basket_datas};module.exports.getStats=function(a,b,d){Baskets.find({},function(a,e){console.log("err :::: ",a);console.log("infos ::::: ",e);a?d({status:405,datas:a}):d({status:200,datas:e})})};
module.exports.get=function(a,b,d){console.log("AUTH REQ ??????? ",a);var c={};if(a.isAdmin&&"undefined"!==typeof a.basket_id)c={_id:a.basket_id};else if("undefined"!==typeof a.options)"undefined"!==typeof a.options.user_id&&(c={user_id:a.options.user_id});else if("undefined"!==typeof a.user_id)c={user_id:a.user_id};else if("undefined"!==typeof b.session.Auth)c={user_id:b.session.Auth._id};else return d({status:401,message:"NOT_LOGGED_IN"}),!1;Baskets.find(c,function(a,c){a?d({status:401,datas:a}):
(0===c.length&&d({status:200,datas:c}),c.forEach(function(a){var e=a.total_amount=0;0<a.products.length?a.products.forEach(function(g){products_controller.get({product_id:g.product_id},b,function(b){0===b.datas.length?g.infos={label:"DOSNT_EXIST",message:"PRODUIT_INTROUVABLE_OU_SUPPRIME"}:(g.infos=b.datas[0],a.total_amount+=g.price*("undefined"!==typeof g.quantity?g.quantity:1));e++;e>=a.products.length&&d({status:200,datas:c})})}):d({status:200,datas:c})}))})};
...
module.exports.create=function(a,b,d){var c=this,e=null;console.log("datas basket model create ",a);products_controller.get(a,b,function(f){if(200===f.status){e=f.datas[0];if("undefined"===typeof a.options.user_id)return d({status:405,message:"user not defined"}),!1;c.get(a,b,function(c){console.log("SELF GET ",c);if(200===c.status&&0<c.datas.length){if(c=c.datas[0],"undefined"!==typeof a.product_id){var b=_.where(c.products,{product_id:a.product_id});0<b.length?(b=b[0],b.quantity=1,b.total=parseFloat(b.price)*
parseInt(b.quantity),b.updated=Date.now()):c.products.push({product_id:a.product_id,datas:a.datas,price:e.price,total:(parseFloat(e.price)*parseInt(a.quantity)).toFixed(2),created:Date.now(),updated:Date.now()});c.updated=Date.now();Baskets.updateOne({_id:c._id},{$set:c},function(a,c){a?d({status:405,message:a}):d({status:200,datas:c})})}}else c={user_id:a.options.user_id,products:[{product_id:a.product_id,quantity:1,datas:a.datas,price:e.price,total:(parseFloat(e.price)*parseFloat(a.quantity)).toFixed(2),
created:Date.now(),updated:Date.now()}],total:(parseFloat(e.price)*parseFloat(a.quantity)).toFixed(2),created:Date.now(),updated:Date.now()},new_basket=new Baskets(c),new_basket.save(function(a,c){a?d({status:405,datas:{message:"ERROR",infos:a}}):d({status:200,datas:c})})})}else d(f)})};module.exports.update=function(a,b,d){delete datas.options;delete datas._id;datas.updated=Date.now();Baskets.updateOne({_id:a.body._id},{$set:datas},function(a,b){a?d({status:405,datas:a}):d({status:200,datas:b})})};
module.exports.deleteUserBasket=function(a,b){Baskets.deleteOne({user_id:a},function(a){b(a)})};module.exports["delete"]=function(a,b,d){"undefined"!==typeof a.body.product_id&&console.log("req.body.product_id ",a.body.product_id);"undefined"!==typeof a.body.basket_id&&console.log("req.body.basket_id ",a.body.basket_id);Baskets.update({_id:a.body.basket_id},{$pull:{products:{product_id:a.body.product_id}}},function(a,b){a?d({status:400,datas:{message:a}}):d({status:200,datas:b})})};
