...
router.all("*",function(a,d,b){var c=(a.headers["x-forwarded-proto"]||"").toLowerCase();console.log("HAS HTTPS ",c);console.log("req.get('host') ",a.get("host"));console.log("req.get('origin') ",a.get("origin"));console.log("req.get('X-Forwarded-Host') >>>>> ",a.get("X-Forwarded-Host"));c=(a.headers["x-forwarded-proto"]||"").toLowerCase();"https"===c?(console.log("HAS HTTPS ",c),b()):"undefined"!==typeof a.get("X-Forwarded-Host")?-1===a.get("X-Forwarded-Host").indexOf("127.0.0.1")?d.redirect("https://"+
a.get("X-Forwarded-Host")+a.url):b():b()});router.use("/admin",admin);router.use("/admin/:any",admin);router.use("/users",users);router.use("/me",me);router.use("/account",account);router.use("/account/:any",account);router.use("/api",api);router.use("/media",media);router.use("/gmail",gmail);router.use("/address",address);router.use("/validation",validation);router.use("/templating",templating);router.use("/redirect",redirect);router.use("/auth",auth);router.use("/checking_session",checking_session);
router.use("/products",products);router.use("/settings",settings);router.use("/basket",basket);router.use("/orders",orders);router.use("/comments",comments);router.use("/stripe_test",stripe_test);router.use("/coupon_code",coupon_code);router.use("/coupon_code/:any",coupon_code);router.use("/",index);app.use("/",router);app.use(function(a,d,b){a=Error("Not Found");a.status=404;b(a)});
app.use(function(a,d,b,c){b.locals.message=a.message;b.locals.error="development"===d.app.get("env")?a:{};b.status(a.status||500);b.render("error")});
