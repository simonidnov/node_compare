...
const transactionsSchemas = new db.Schema(transaction_datas),
      Transactions = db.model('Transactions', transactionsSchemas);

module.exports = {
    attributes: transaction_datas
};
module.exports.get = function(req, res, callback) {
    var query = {};
    Transactions.find(query, function(err, infos){
        if(err){
...
module.exports.create = function(req, res, callback) {
    var datas = {};
    new_transaction = new Orders(datas);
    new_transaction.save(function(err, infos){
        if(err){
