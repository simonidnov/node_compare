var express = require('express'),
    wallets = express.Router(),
    Wallets_controller = require('../controllers/wallets_controller'),
    Auth_helper = require('../helpers/auth_helper'),
    language_helper = require('../helpers/languages_helper'),
    uri_helper = require('../helpers/uri_helper'),
    lang = require('../public/languages/auth_lang');

wallets.use(function(req, res, next){
    //ACCEPT CORS
...
wallets
    .get('/', function(req, res, next) {
        Wallets_controller.get(req, res, function(e){
            res.status(e.status).send(e.datas);
        });
    })
    .get('/amount', function(req, res, next) {
        Wallets_controller.getAmount(req, res, function(e){
            res.status(e.status).send(e.datas);
        });
    })
    .post('/', function(req, res, next) {
        Wallets_controller.create(req.body.data, res, function(e){
    ...
    .put('/', function(req, res, next) {
        Wallets_controller.update(req.body.data, res, function(e){
            res.status(e.status).send({status:e.status, infos:e.datas});
        });
    })
    .delete('/', function(req, res, next) {
        Wallets_controller.delete(req, res, function(e){
            res.status(e.status).send({status:e.status, infos:e.datas});
        });
    });
