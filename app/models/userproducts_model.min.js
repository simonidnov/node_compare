...
module.exports.get=function(a,e,c){var b={};"undefined"!==typeof a.session.Auth?b={user_id:a.session.Auth._id}:"undefined"!==typeof a.query.user_id?b={user_id:datas.user_id}:"undefined"!==typeof a.query.options&&(b={user_id:a.query.options.user_id});if("undefined"===typeof b.user_id)return c({status:401,datas:{message:"UNAUTHORISED_NEED_USER"}}),!1;a.user_id=b.user_id;this.checkOrders(a,e,function(a){Userproducts.find(b,function(a,b){a?c({status:405,datas:a}):c({status:200,datas:b});return!1}).sort({created:-1})})};
module.exports.allreadyBuy=function(a,e,c){Userproducts.findOne({user_id:a,product_id:e},function(a,d){a?c({status:404,message:"NEVER_BUY"}):null===d?c({status:404,message:"NEVER_BUY",datas:d}):c({status:200,message:"ALREADY_BUY",datas:d})})};
module.exports.checkOrders=function(a,e,c){var b=this;Orders_model.get(a,e,function(c){if(200===c.status)for(var d=0;d<c.datas.length;d++)for(var h=0;h<c.datas[d].basketdatas.products.length;h++)b.create({user_id:a.user_id,product_id:c.datas[d].basketdatas.products[h].product_id},e,function(a){})});c({status:200,message:"CHECKING"})};
...
m=require("../controllers/auth_controller");f.forEach(function(b,f){a.query.user={_id:b.user_id};m.getUserInfos(a,e,function(e){if(200===e.status){var f=e.user[0];(new Date(b.share_date)).toLocaleDateString("fr-FR",{month:"long",day:"numeric"});l.sendMaChansonEcard(a,{subject:"Bonjour "+b.share_name+", "+f.pseudo+" vous souhaites un joyeux anniversaire en chanson",title:"Bonjour "+b.share_name+", "+f.pseudo+" vous souhaites un joyeux anniversaire en chanson",name:b.share_name,friend:f.pseudo,friend_email:f.email,
default_message:"Bonjour "+b.share_name+",<br>votre ami(e) "+f.pseudo+" vous souhaites un joyeux anniversaire en chanson !",message:b.share_message,product_id:b.product_id,share_id:b._id,email:f.email,to:b.share_email},function(a){200===a.status&&(Usershares.update({_id:b._id},{$set:{share_sent:!0}},function(b,a){console.log("SUCCESS SAVE SENT DATAS TRUE")}),k++,l.send(null,{subject:"La chanson d'anniversaire personnalis\u00e9e pour "+b.share_name+" \u00e0 bien \u00e9t\u00e9 envoy\u00e9e",title:"La chanson d'anniversaire personnalis\u00e9e pour "+
b.share_name+" \u00e0 bien \u00e9t\u00e9 envoy\u00e9e",message:"Bonjour "+f.pseudo+"<br>Vous avez envoy\u00e9 une chanson d'anniversaire pour votre ami(e) "+b.share_name+".<br>Cet email confirme l'envoie de votre chanson sur l'adresse email de votre ami(e) : "+b.share_email+"<br>Voici votre message personnalis\u00e9 qui accompagnera la chanson :<br>"+b.share_message,email:"contact@joyvox.fr",share_id:b._id,to:f.email},function(b){}));d++;d===g-1&&c({status:200,message:"EMAILS_SENT",total:g,success:k})})}else d++,
d===g-1&&c({status:200,message:"EMAILS_SENT",total:g,success:k})})})}}})};
...
b.share_name+" \u00e0 bien \u00e9t\u00e9 configur\u00e9",title:"L'envoie de votre chanson d'anniversaire \u00e0 bien \u00e9t\u00e9 configur\u00e9",message:"Bonjour "+e.pseudo+"<br>Vous avez configur\u00e9 l'envoie d'une chanson d'anniversaire pour votre ami(e) "+b.share_name+" pour le "+k+".<br>Cet email confirme que l'email sera envoy\u00e9 \u00e0 9h00 ce jour pr\u00e9cis sur l'adresse email de votre ami(e) : "+b.share_email+"<br>Voici votre message personnalis\u00e9 qui accompagnera la chanson :<br>"+
b.share_message,email:"contact@joyvox.fr",share_id:g._id,to:e.email},function(a){c({status:200,datas:a.datas,response_display:{title:"Partager",message:"Le partage de la chanson d'anniversaire pour "+b.share_name+" a bien \u00e9t\u00e9 configur\u00e9e, elle sera envoy\u00e9 \u00e0 votre ami(e) sur son email "+b.share_email+" le "+k+" !"}})})}else h.sendMaChansonEcard(a,{subject:"Bonjour"+b.share_name+", "+e.pseudo+" vous souhaites un joyeux anniversaire en chanson",title:"Bonjour"+b.share_name+", "+
e.pseudo+" vous souhaites un joyeux anniversaire en chanson",default_message:"Bonjour "+b.share_name+",<br>votre ami(e) "+e.pseudo+" vous souhaites un joyeux anniversaire en chanson !",message:b.share_message,product_id:b.product_id,share_id:g._id,email:e.email,to:b.share_email},function(a){200===a.status?h.send(null,{subject:"La chanson d'anniversaire personnalis\u00e9e pour "+b.share_name+" \u00e0 bien \u00e9t\u00e9 envoy\u00e9e",title:"La chanson d'anniversaire personnalis\u00e9e pour "+b.share_name+
" \u00e0 bien \u00e9t\u00e9 envoy\u00e9e",message:"Bonjour "+e.pseudo+"<br>Vous avez envoy\u00e9 une chanson d'anniversaire pour votre ami(e) "+b.share_name+".<br>Cet email confirme l'envoie de votre chanson sur l'adresse email de votre ami(e) : "+b.share_email+"<br>Voici votre message personnalis\u00e9 qui accompagnera la chanson :<br>"+b.share_message,email:"contact@joyvox.fr",share_id:g._id,to:e.email},function(a){c({status:200,datas:a.datas,response_display:{title:"Partager ma chanson",message:"La chanson a bien \u00e9t\u00e9 envoy\u00e9e \u00e0 "+
b.share_name+" sur son adresse email "+b.share_email+" !"}})}):c({status:203,datas:a.datas,response_display:{title:"Erreur de Partage",message:"Nous n'avons pas r\u00e9ussi \u00e0 envoyer la chanson \u00e0 votre ami(e)."}})})})}else c({status:203,datas:d.datas,response_display:{title:"Erreur de Partage",message:"Nous n'avons pas r\u00e9ussi \u00e0 vous authentifier pour cr\u00e9er l'envoie de votre chanson."}})})}})};
