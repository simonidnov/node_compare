...
function(a,c){a?console.log("impossible d ins\u00e9rer le new_device ",a):console.log("new_device ajout\u00e9 success ",d)}):0===c.length?User.update({_id:f[0]._id},{$push:{devices:d}},function(a,c){a?console.log("impossible d ins\u00e9rer le new_device ",a):console.log("new_device ajout\u00e9 success ",d)}):User.update({id:f[0]._id,devices:{$elemMatch:{uid:device_uid}}},{device:{$set:d}},function(a,c){a?console.log("update device token error ",a):console.log("update device token success ",c)},!0)}));
var h=f[0].avatar,h=""===f[0].avatar||null==f[0].avatar?""===b.avatar||null==b.avatar||"undefined"==typeof b.avatar?gravatar.url(f[0].email,{s:"200",r:"pg",d:"404"}).replace("//","http://"):b.avatar:f[0].avatar;urlExists(h,function(b,d){d||(h=app.locals.settings.host+"/public/images/assets/account.svg");User.update({_id:f[0]._id},{$set:{updated:Date.now(),avatar:h}},function(b,d){b?c({status:400,code:b.code,error:b,message:"USER_LOGIN_ERROR"}):e.reset_session(a,f[0]._id,function(a){a.avatar=a.avatar;
c({status:200,message:"USER_LOGIN_SUCCESS",idkids_user:a})})})})}}):c({status:400,message:"USER_WRONG_EMAIL"}):c({status:400,message:"USER_LOGIN_ERROR"});return b};module.exports.logout=function(a,b,c){"undefined"!==typeof a.session.Auth&&a.session.destroy();c({status:200,datas:{message:"SESSION_DELETED"}})};
...
module.exports.check_user=function(a,b){jwt.sign({secret:a.options.user_secret},config.secrets.global.secret,{expiresIn:"2 days"});jwt.verify(a.options.user_token,config.secrets.global.secret,function(c,d){c?b({status:203,message:"UNAUTHORISED_TOKEN",response_display:{title:"Session invalide",message:"Vous n'\u00eates plus connect\u00e9.<br>veuillez vous reconnecter."},datas:c}):User.find({_id:a.options.user_id,token:a.options.user_token},function(c,d){c?b({status:203,message:"UNAUTHORISED_TOKEN",
datas:c,response_display:{title:"Session invalide",message:"Vous n'\u00eates plus connect\u00e9.<br>veuillez vous reconnecter."}}):0===d.length?b({status:203,message:"UNAUTHORISED",response_display:{title:"Session invalide",message:"Vous n'\u00eates plus connect\u00e9.<br>veuillez vous reconnecter."}}):jwt.verify(a.options.user_token,config.secrets.global.secret,function(c,e){c?User.update({id:a.options.user_id},{$set:{updated:Date.now()}},function(a,c){a?b({status:401,message:"CANT_UPDATE_TOKEN",
datas:a}):b({status:200,message:"TOKEN_UPDATED",datas:d})},!0):b({status:200,message:"TOKEN_UPDATED",datas:d})})})})};module.exports.checking_session=function(a,b,c){var d=this;User.findOne({_id:a.query._id,token:a.query.token,secret:a.query.secret},function(b,g){b?c({status:401,code:b.code,error:b,message:b.message}):d.reset_session(a,a.query._id,c)})};
