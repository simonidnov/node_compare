...
app.use("/uploads",express["static"](path.join(__dirname,"uploads")));app.use("/public",express["static"](path.join(__dirname,"public")));app.use("/node_modules",express["static"](path.join(__dirname,"node_modules")));app.use(logger("dev"));app.use(bodyParser.json());app.use(bodyParser.urlencoded({extended:!0}));app.use(cookieParser());
app.use(function(a,c,b){"https"===(a.headers["x-forwarded-proto"]||"").toLowerCase()?b():-1===a.headers.host.indexOf("127.0.0.1")?c.redirect("https://"+a.headers.host+a.url):b()});app.use(express["static"](path.join(__dirname,"public")));router.use("/admin",admin);router.use("/admin/:any",admin);router.use("/users",users);router.use("/me",me);router.use("/account",account);router.use("/account/:any",account);router.use("/api",api);router.use("/media",media);router.use("/gmail",gmail);
router.use("/address",address);router.use("/validation",validation);router.use("/templating",templating);router.use("/redirect",redirect);router.use("/auth",auth);router.use("/checking_session",checking_session);router.use("/products",products);router.use("/settings",settings);router.use("/basket",basket);router.use("/orders",orders);router.use("/comments",comments);router.use("/stripe_test",stripe_test);router.use("/coupon_code",coupon_code);router.use("/coupon_code/:any",coupon_code);
