...
orders.get("/",function(c,b,d){Orders_controller.get(c,b,function(a){b.status(a.status).send(a.datas)})}).get("/amount",function(c,b,d){Orders_controller.getAmount(c,b,function(a){b.status(a.status).send(a.datas)})}).post("/transaction",function(c,b,d){Orders_controller.createCharge(c,b,function(a){a.title="Paiement";console.log(a);b.status(a.status).render("order",a)})}).post("/",function(c,b,d){Orders_controller.create(c.body.data,b,function(a){200===a.status?b.status(a.status).send({status:a.status,
response_display:{title:"Commande",message:"Votre commande est pr\u00eate \u00e0 \u00eatre r\u00e9gl\u00e9e.",buttons:["OK"]}}):b.status(a.status).send(a)})}).put("/",function(c,b,d){Orders_controller.update(c.body.data,b,function(a){b.status(a.status).send({status:a.status,infos:a.datas})})})["delete"]("/",function(c,b,d){Orders_controller["delete"](c,b,function(a){b.status(a.status).send({status:a.status,infos:a.datas})})});module.exports=orders;
