...
else{if(0===e.length)return User.find({email:b.email},function(a,b){a?c({status:400,code:11,error:a,message:"USER_WRONG_EMAIL"}):0===b.length?c({status:400,code:11,error:a,message:"USER_WRONG_EMAIL",email_valid:b.length}):c({status:400,code:13,error:a,message:"USER_WRONG_PASSWORD",email_valid:b.length})}),!1;var g=jwt.sign({secret:e[0].secret,email:e[0].email,password:b.password},config.secrets.global.secret,{expiresIn:"2 days"}),h=e[0].avatar;console.log("AVATAR ",h);console.log("AVATAR ",h);console.log("AVATAR ",
h);console.log("AVATAR ",h);console.log("AVATAR ",h);h=""===e[0].avatar||null===e[0].avatar?""===b.avatar||null===b.avatar||"undefined"===typeof b.avatar?gravatar.url(e[0].email,{protocol:"https",s:"100"}):b.avatar:e[0].avatar;urlExists(h,function(b,f){f||console.log("AVATAR RESET ? ",h);User.update({_id:e[0]._id},{$set:{updated:Date.now(),avatar:h,token:g}},function(b,f){b?c({status:400,code:b.code,error:b,message:"USER_LOGIN_ERROR"}):d.reset_session(a,e[0]._id,function(a){a.avatar=h;c({status:200,
