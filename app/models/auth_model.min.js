...
c({status:400,code:11,error:a,message:"USER_WRONG_EMAIL"}):0===b.length?c({status:400,code:11,error:a,message:"USER_WRONG_EMAIL",email_valid:b.length}):c({status:400,code:13,error:a,message:"USER_WRONG_PASSWORD",email_valid:b.length})}),!1;var k=jwt.sign({secret:f[0].secret},config.secrets.global.secret,{expiresIn:"2 days"});"undefined"!==typeof a.query.remember_me&&(d.token=k,User.find({_id:f[0]._id,devices:{$elemMatch:{uid:device_uid}}},function(a,c){a?User.update({_id:f[0]._id},{$push:{devices:d}},
...
c)},!0)}));var h=f[0].avatar,h=""===f[0].avatar||null==f[0].avatar?""===b.avatar||null==b.avatar||"undefined"==typeof b.avatar?gravatar.url(f[0].email,{s:"200",r:"pg",d:"404"}).replace("//","http://"):b.avatar:f[0].avatar;urlExists(h,function(b,d){d||(h=app.locals.settings.host+"/public/images/assets/account.svg");User.update({_id:f[0]._id},{$set:{token:k,updated:Date.now(),avatar:h}},function(b,d){b?c({status:400,code:b.code,error:b,message:"USER_LOGIN_ERROR"}):e.reset_session(a,f[0]._id,function(a){a.avatar=
a.avatar;c({status:200,message:"USER_LOGIN_SUCCESS",idkids_user:a})})})})}}):c({status:400,message:"USER_WRONG_EMAIL"}):c({status:400,message:"USER_LOGIN_ERROR"});return b};module.exports.logout=function(a,b,c){"undefined"!==typeof a.session.Auth&&a.session.destroy();c({status:200,datas:{message:"SESSION_DELETED"}})};
module.exports.register=function(a,b){if("undefined"===typeof a.body||"undefined"===typeof a.body.subscribe_password)return!1;var c=sha1(a.body.subscribe_password);db.connect(config.database.users,{useMongoClient:!0});var d=this,c={email:a.body.subscribe_email,password:c,pseudo:a.body.pseudo,secret:jwt.sign({},config.secrets.global.secret,{},{expiresIn:"2 days"}),termAccept:!0,rights:{type:"R",authorizations:["me"]}};c.token=jwt.sign({secret:c.secret},config.secrets.global.secret,{expiresIn:"2 days"});
c.device=[{uid:device_uid,token:jwt.sign({secret:c.secret},config.secrets.global.secret,{expiresIn:"2 days"}),avatar:gravatar.url(a.body.subscribe_email,{s:"200",r:"pg",d:"404"},!0).replace("//","http://")}];if(a.body.subscribe_newsletter){if(c.newsletter=!0,c.newsletter_services={},"undefined"!==typeof app.locals&&"undefined"!==typeof app.locals.applications)for(var e=0;e<app.locals.applications.length;e++)a.body["newsletter_"+app.locals.applications[e].short_name]&&(c.newsletter_services[app.locals.applications[e].short_name]=
1)}else c.newsletter=!1,c.newsletter_services={};new_user=new User(c);new_user.save(function(c,e){c?b({status:400,message:"SUBSCRIBE_EMAIL_EXIST",err:c}):d.reset_session(a,e._id,function(a){b({status:200,user:e})})})};
module.exports.lost_password=function(a,b,c){"unefined"===typeof a.body.email&&c({status:403,message:"UNKNOW_USER_EMAIL"});var d=this;User.findOne({email:a.body.email},function(b,g){b?c({status:403,datas:{message:b}}):null===g?c({status:403,message:"UNKNOW_USER_EMAIL"}):d.getValidationCode(g._id,function(b){200===b.status?Email_controller.lost_password(a,b,function(a){console.log("----------------------- ");console.log(a);console.log("----------------------- ");c({status:a.status,response_display:{message:"REQUEST_PASSWORD_MESSAGE",
title:"REQUEST_PASSWORD_TITLE"},message:"PASSWORD_SENDED"})}):c({status:b.status,datas:b})})})};module.exports.unregister=function(a,b,c){User.remove({_id:a.body.id},function(){c({status:200,message:"USER_DELETED",infos:a.body})});return a.body};
module.exports.update=function(a,b,c,d){device_uid=a.body.device_uid;var e=this;c.updated=Date.now();"undefined"!==typeof c.birthDate&&(c.birthDate=c.birthDate);User.update({_id:b},{$set:c},function(c,f){c?d({status:401,message:"CANT_UPDATE_TOKEN",datas:c}):e.reset_session(a,b,function(){d({status:200,message:"TOKEN_UPDATED",datas:f,response_display:{title:"Mis \u00e0 jour",message:"Votre profil vient d'\u00eatre mis \u00e0 jour."}})})},!0)};
module.exports.updatePassword=function(a,b,c){User.update({email:a.email},{$set:{password:sha1(a.password)}},function(a,b){a?c({status:401,message:"CANT_UPDATE_PASSWORD",error:a}):c({status:200,message:"PASSWORD_UPDATED",infos:b})})};module.exports.check_user_session=function(a,b){};
module.exports.check_user=function(a,b){var c=jwt.sign({secret:a.options.user_secret},config.secrets.global.secret,{expiresIn:"2 days"});jwt.verify(a.options.user_token,config.secrets.global.secret,function(d,e){d?b({status:203,message:"UNAUTHORISED_TOKEN",response_display:{title:"Session invalide",message:"Vous n'\u00eates plus connect\u00e9.<br>veuillez vous reconnecter."},datas:d}):User.find({_id:a.options.user_id,token:a.options.user_token},function(d,e){d?b({status:203,message:"UNAUTHORISED_TOKEN",
datas:d,response_display:{title:"Session invalide",message:"Vous n'\u00eates plus connect\u00e9.<br>veuillez vous reconnecter."}}):0===e.length?b({status:203,message:"UNAUTHORISED",response_display:{title:"Session invalide",message:"Vous n'\u00eates plus connect\u00e9.<br>veuillez vous reconnecter."}}):jwt.verify(a.options.user_token,config.secrets.global.secret,function(d,g){d?User.update({id:a.options.user_id},{$set:{token:c,updated:Date.now()}},function(a,d){a?b({status:401,message:"CANT_UPDATE_TOKEN",
datas:a}):b({status:200,message:"TOKEN_UPDATED",datas:e,updated_token:c})},!0):b({status:200,message:"TOKEN_UPDATED",datas:e})})})})};module.exports.checking_session=function(a,b,c){var d=this;User.findOne({_id:a.query._id,token:a.query.token,secret:a.query.secret},function(b,g){b?c({status:401,code:b.code,error:b,message:b.message}):d.reset_session(a,a.query._id,c)})};
