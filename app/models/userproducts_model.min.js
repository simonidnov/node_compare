...
module.exports.get=function(a,d,c){var b={};"undefined"!==typeof a.session.Auth?b={user_id:a.session.Auth._id}:"undefined"!==typeof a.query.user_id?b={user_id:datas.user_id}:"undefined"!==typeof a.query.options&&(b={user_id:a.query.options.user_id});if("undefined"===typeof b.user_id)return c({status:401,datas:{message:"UNAUTHORISED_NEED_USER"}}),!1;"undefined"!==typeof a.query.product_id&&(b.product_id=a.query.product_id);a.user_id=b.user_id;this.checkOrders(a,d,function(a){Userproducts.find(b,function(a,
b){a?c({status:405,datas:a}):c({status:200,datas:b});return!1}).sort({created:-1})})};module.exports.allreadyBuy=function(a,d,c){Userproducts.findOne({user_id:a,product_id:d},function(a,e){a?c({status:404,message:"NEVER_BUY"}):null===e?c({status:404,message:"NEVER_BUY",datas:e}):c({status:200,message:"ALREADY_BUY",datas:e})})};
module.exports.checkOrders=function(a,d,c){var b=this;Orders_model.get(a,d,function(c){if(200===c.status)for(var e=0;e<c.datas.length;e++)for(var h=0;h<c.datas[e].basketdatas.products.length;h++)b.create({user_id:a.user_id,product_id:c.datas[e].basketdatas.products[h].product_id},d,function(a){})});c({status:200,message:"CHECKING"})};
...
module.exports.getShare=function(a,d,c){Usershares.find({share_email:a.params.user_email,product_id:a.params.product_id,_id:a.params.share_id},function(a,e){a?c({status:404,message:"UNKNOW_SHARE_CONTENT"}):0===e.length?c({status:404,message:"UNKNOW_SHARE_CONTENT"}):Products_model.get({product_id:e[0].product_id},d,function(a){c({status:200,datas:{share:e[0],product:a.datas[0]}})})})};
module.exports.shares_of_the_day=function(a,d,c){var b=new Date;Usershares.find({share_date:{$not:{$type:9}}},function(a,b){b.forEach(function(a,b){null===a.share_date&&(a.share_date=new Date);a.share_date=new Date(a.share_date);Usershares.update({_id:a._id},{$set:{share_date:a.share_date}},function(a,b){console.log("err ",a);console.log("updates ",b)})})});Usershares.aggregate([{$project:{day:{$dayOfMonth:"$share_date"},month:{$month:"$share_date"},year:{$year:"$share_date"},user_id:"$user_id",product_id:"$product_id",
share_name:"$share_name",share_email:"$share_email",share_message:"$share_message",share_link:"$share_link",share_later:"$share_later",share_date:"$share_date",share_sent:"$share_sent",created:"$created",updated:"$updated"}},{$match:{day:b.getDate(),month:b.getMonth()+1,share_later:!0}}],function(b,f){if(b)c({status:500,message:"ERROR_OCCURED_REQUEST",err:b});else{var e=0,g=f.length,k=0;if(0===f.length)c({status:200,message:"NO_EMAILS_FOR_TODAY",shares:f});else{var l=require("../controllers/email_controller"),
m=require("../controllers/auth_controller");f.forEach(function(b,f){a.query.user={_id:b.user_id};m.getUserInfos(a,d,function(d){if(200===d.status){var f=d.user[0];(new Date(b.share_date)).toLocaleDateString("fr-FR",{month:"long",day:"numeric"});l.sendMaChansonEcard(a,{subject:"Bonjour "+b.share_name+", "+f.pseudo+" vous souhaite un joyeux anniversaire en chanson",title:"Bonjour "+b.share_name+", "+f.pseudo+" vous souhaite un joyeux anniversaire en chanson",name:b.share_name,friend:f.pseudo,friend_email:f.email,
default_message:"Bonjour "+b.share_name+",<br>votre ami(e) "+f.pseudo+" vous souhaite un joyeux anniversaire en chanson !",message:b.share_message,product_id:b.product_id,share_id:b._id,email:f.email,to:b.share_email},function(a){200===a.status&&(Usershares.update({_id:b._id},{$set:{share_sent:!0}},function(a,b){console.log("SUCCESS SAVE SENT DATAS TRUE")}),k++,l.send(null,{subject:"La chanson d'anniversaire personnalis\u00e9e pour "+b.share_name+" \u00e0 bien \u00e9t\u00e9 envoy\u00e9e",title:"La chanson d'anniversaire personnalis\u00e9e pour "+
b.share_name+" \u00e0 bien \u00e9t\u00e9 envoy\u00e9e",message:"Bonjour "+f.pseudo+"<br>Vous avez envoy\u00e9 une chanson d'anniversaire pour votre ami(e) "+b.share_name+".<br>Cet email confirme l'envoi de votre chanson sur l'adresse email de votre ami(e) : "+b.share_email+"<br>Voici votre message personnalis\u00e9 qui accompagnera la chanson :<br>"+b.share_message,email:"contact@joyvox.fr",share_id:b._id,to:f.email},function(a){}));e++;e===g-1&&c({status:200,message:"EMAILS_SENT",total:g,success:k})})}else e++,
...
else{var h=require("../controllers/email_controller"),g=require("../controllers/auth_controller");a.query.user={_id:b.options.user_id};g.getUserInfos(a,d,function(e){if(200===e.status){var d=e.user[0];new_shareproduct=new Usershares({user_id:e.user[0]._id,product_id:b.product_id,share_name:b.share_name,share_email:b.share_email,share_message:b.share_message,share_link:b.share_link,share_date:b.share_date?b.share_date:new Date,share_later:b.share_later});new_shareproduct.save(function(f,g){if(f)c({status:200,
...
b.share_name+" \u00e0 bien \u00e9t\u00e9 configur\u00e9",title:"L'envoi de votre chanson d'anniversaire \u00e0 bien \u00e9t\u00e9 configur\u00e9",message:"Bonjour "+d.pseudo+"<br>Vous avez configur\u00e9 l'envoi d'une chanson d'anniversaire pour votre ami(e) "+b.share_name+" pour le "+k+".<br>Cet email confirme que l'email sera envoy\u00e9 \u00e0 9h00 ce jour pr\u00e9cis sur l'adresse email de votre ami(e) : "+b.share_email+"<br>Voici votre message personnalis\u00e9 qui accompagnera la chanson :<br>"+
b.share_message,email:"contact@joyvox.fr",share_id:g._id,to:d.email},function(a){c({status:200,datas:a.datas,response_display:{title:"Partager",message:"Le partage de la chanson d'anniversaire pour "+b.share_name+" a bien \u00e9t\u00e9 configur\u00e9e, elle sera envoy\u00e9 \u00e0 votre ami(e) sur son email "+b.share_email+" le "+k+" !"}})})}else h.sendMaChansonEcard(a,{subject:"Bonjour "+b.share_name+", "+d.pseudo+" vous souhaite un joyeux anniversaire en chanson",title:"Bonjour "+b.share_name+", "+
d.pseudo+" vous souhaite un joyeux anniversaire en chanson",default_message:"Bonjour "+b.share_name+",<br>votre ami(e) "+d.pseudo+" vous souhaite un joyeux anniversaire en chanson !",message:b.share_message,friend:d.pseudo,friend_email:d.email,product_id:b.product_id,share_id:g._id,email:d.email,to:b.share_email},function(a){200===a.status?h.send(null,{subject:"La chanson d'anniversaire personnalis\u00e9e pour "+b.share_name+" \u00e0 bien \u00e9t\u00e9 envoy\u00e9e",title:"La chanson d'anniversaire personnalis\u00e9e pour "+
b.share_name+" \u00e0 bien \u00e9t\u00e9 envoy\u00e9e",message:"Bonjour "+d.pseudo+"<br>Vous avez envoy\u00e9 une chanson d'anniversaire pour votre ami(e) "+b.share_name+".<br>Cet email confirme l'envoi de votre chanson sur l'adresse email de votre ami(e) : "+b.share_email+"<br>Voici votre message personnalis\u00e9 qui accompagnera la chanson :<br>"+b.share_message,email:"contact@joyvox.fr",share_id:g._id,to:d.email},function(a){c({status:200,datas:a.datas,response_display:{title:"Partager ma chanson",
message:"La chanson a bien \u00e9t\u00e9 envoy\u00e9e \u00e0 "+b.share_name+" sur son adresse email "+b.share_email+" !"}})}):c({status:203,datas:a.datas,response_display:{title:"Erreur de Partage",message:"Nous n'avons pas r\u00e9ussi \u00e0 envoyer la chanson \u00e0 votre ami(e)."}})})})}else c({status:203,datas:e.datas,response_display:{title:"Erreur de Partage",message:"Nous n'avons pas r\u00e9ussi \u00e0 vous authentifier pour cr\u00e9er l'envoi de votre chanson."}})})}})};
