...
b,c){require("fs").existsSync("./uploads/"+a.params.filename.replace(".mp3","_shortcut15.mp3"))?(a.params.filename=a.params.filename.replace(".mp3","_shortcut15.mp3"),c()):require("fluent-ffmpeg")("./uploads/"+a.params.filename).duration(15).toFormat("mp3").audioFilters([{filter:"afade",options:"t=out:st=13:d=2"}]).on("error",function(a){b.status("400").send(a)}).on("progress",function(a){console.log("Processing: "+a.targetSize+" KB converted")}).on("end",function(){a.params.filename=a.params.filename.replace(".mp3",
