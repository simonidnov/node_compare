...
module.exports.create=function(a,b,d){var c=this,e=null;products_controller.get(a,b,function(f){if(200===f.status){e=f.datas[0];if("undefined"===typeof a.options.user_id)return d({status:405,message:"user not defined"}),!1;c.get(a,b,function(c){if(200===c.status&&0<c.datas.length){if(c=c.datas[0],"undefined"!==typeof a.product_id){var b=_.where(c.products,{product_id:a.product_id});0<b.length?(b=b[0],b.quantity=1,b.total=parseFloat(b.price)*parseInt(b.quantity),b.updated=Date.now()):(console.log("datas PRODUCT BASKET ================== ",
a),c.products.push({product_id:a.product_id,datas:a.datas,url:a.url,price:e.price,total:(parseFloat(e.price)*parseInt(a.quantity)).toFixed(2),created:Date.now(),updated:Date.now()}));c.updated=Date.now();Baskets.updateOne({_id:c._id},{$set:c},function(a,c){a?d({status:405,message:a}):d({status:200,datas:c})})}}else c={user_id:a.options.user_id,products:[{product_id:a.product_id,quantity:1,datas:a.datas,price:e.price,total:(parseFloat(e.price)*parseFloat(a.quantity)).toFixed(2),created:Date.now(),
updated:Date.now()}],total:(parseFloat(e.price)*parseFloat(a.quantity)).toFixed(2),created:Date.now(),updated:Date.now()},new_basket=new Baskets(c),new_basket.save(function(a,c){a?d({status:405,datas:{message:"ERROR",infos:a}}):d({status:200,datas:c})})})}else d(f)})};
module.exports.update=function(a,b,d){delete a.options;delete a.decoded;delete a.isAdmin;delete a.updated_token;delete a.device_infos;b=a.basket_id;delete a.basket_id;console.log("UPDATE REQ ",a);a.updated=Date.now();Baskets.update({_id:b},{$set:a},function(a,b){console.log("err ",a);a?d({status:405,datas:a}):d({status:200,datas:b})})};module.exports.deleteUserBasket=function(a,b){Baskets.deleteOne({user_id:a},function(a){b(a)})};
