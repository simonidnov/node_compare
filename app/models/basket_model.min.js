...
total:{type:"number","default":0},created:{type:"Date","default":Date.now},updated:{type:"Date","default":Date.now}};0===db.connection.readyState&&db.connect(config.database.users,{useMongoClient:!0});var basketSchemas=new db.Schema(basket_datas),Baskets=db.model("Baskets",basketSchemas);module.exports={attributes:basket_datas};module.exports.getStats=function(a,b,d){Baskets.find({},function(a,b){a?d({status:405,datas:a}):d({status:200,datas:b})})};
...
parseFloat(a.quantity)).toFixed(2),created:Date.now(),updated:Date.now()},new_basket=new Baskets(b),new_basket.save(function(a,b){a?d({status:405,datas:{message:"ERROR",infos:a}}):d({status:200,datas:b})})})}else d(f)})};module.exports.update=function(a,b,d){delete a.options;delete a.decoded;delete a.isAdmin;delete a.updated_token;delete a.device_infos;b=a.basket_id;delete a.basket_id;a.updated=Date.now();Baskets.update({_id:b},{$set:a},function(a,b){a?d({status:405,datas:a}):d({status:200,datas:b})})};
module.exports.deleteUserBasket=function(a,b){Baskets.deleteOne({user_id:a},function(a){b(a)})};module.exports["delete"]=function(a,b,d){Baskets.update({_id:a.body.basket_id},{$pull:{products:{product_id:a.body.product_id}}},function(a,b){a?d({status:400,datas:{message:a}}):d({status:200,datas:b})})};
