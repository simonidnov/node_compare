...
null===b.avatar||"undefined"===typeof b.avatar?gravatar.url(e[0].email,{protocol:"https",s:"100"}):b.avatar:e[0].avatar;urlExists(h,function(b,f){f||(h=app.locals.settings.host+"/public/images/assets/account.svg");User.update({_id:e[0]._id},{$set:{updated:Date.now(),avatar:h,token:g}},function(b,f){b?c({status:400,code:b.code,error:b,message:"USER_LOGIN_ERROR"}):d.reset_session(a,e[0]._id,function(a){a.avatar=a.avatar;c({status:200,message:"USER_LOGIN_SUCCESS",idkids_user:a})})})})}}):c({status:400,
...
!1;User.find({email:d.email,password:sha1(d.password)},function(c,e){if(c)b({status:203,message:"UNAUTHORISED_TOKEN",datas:c,response_display:{title:"Connexion recquise",message:"Vous devez \u00earte connect\u00e9 pour effectuer cette action."}});else if(0===e.length)b({status:203,message:"UNAUTHORISED",response_display:{title:"Connexion recquise",message:"Vous devez \u00earte connect\u00e9 pour effectuer cette action."}});else{var f=jwt.sign({secret:e[0].user_secret,email:e[0].email,password:d.password},
...
module.exports.reset_session=function(a,b,c){console.log("--------------- reset_session ",b);User.findOne({_id:b},function(d,f){d?c({status:401,code:d.code,error:d,message:d.message}):(user_infos=JSON.parse(JSON.stringify(f)),user_infos.current_device=device_uid,Members_model.get(b,null,function(d){user_infos.members=d.datas;Address_model.get(b,null,function(b){user_infos.address=b.datas;console.log("--------------- user_infos RESET SESSION ",user_infos);a.session.Auth=user_infos;c({status:200,message:"SESSION_UPDATED",
