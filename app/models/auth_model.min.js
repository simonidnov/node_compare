...
null===b.avatar||"undefined"===typeof b.avatar?gravatar.url(f[0].email,{s:"200",r:"pg",d:"404"}).replace("//","http://"):b.avatar:f[0].avatar;urlExists(g,function(b,e){e||(g=app.locals.settings.host+"/public/images/assets/account.svg");User.update({_id:f[0]._id},{$set:{updated:Date.now(),avatar:g,token:h}},function(b,e){b?c({status:400,code:b.code,error:b,message:"USER_LOGIN_ERROR"}):d.reset_session(a,f[0]._id,function(a){a.avatar=a.avatar;c({status:200,message:"USER_LOGIN_SUCCESS",idkids_user:a})})})})}}):
c({status:400,message:"USER_WRONG_EMAIL"}):c({status:400,message:"USER_LOGIN_ERROR"});return b};module.exports.logout=function(a,b,c){"undefined"!==typeof a.session.Auth&&a.session.destroy();c({status:200,datas:{message:"SESSION_DELETED"}})};
module.exports.register=function(a,b){if("undefined"===typeof a.body||"undefined"===typeof a.body.subscribe_password)return!1;User.find({email:a.body.subscribe_email},function(a,c){if(!a&&0<c.length)return b({status:203,message:"SUBSCRIBE_EMAIL_EXIST",email_already_exist:c.length}),!1});var c=sha1(a.body.subscribe_password);db.connect(config.database.users,{useMongoClient:!0});var d=this,c={email:a.body.subscribe_email,password:c,pseudo:a.body.pseudo,avatar:gravatar.url(a.body.subscribe_email,{s:"200",
r:"pg",d:"404"},!0).replace("//","http://"),secret:jwt.sign({pseudo:a.body.pseudo+"_"+a.body.subscribe_email},config.secrets.global.secret,{expiresIn:"2 days"}),termAccept:!0,rights:{type:"R",authorizations:["me"]}};c.token=jwt.sign({secret:c.secret,email:a.body.subscribe_email,pazssword:a.body.subscribe_password},config.secrets.global.secret,{expiresIn:"2 days"});c.device=[{uid:device_uid,token:c.token}];if(a.body.subscribe_newsletter){if(c.newsletter=!0,c.newsletter_services={},"undefined"!==typeof app.locals&&
...
module.exports.lost_password=function(a,b,c){"unefined"===typeof a.body.email&&c({status:403,message:"UNKNOW_USER_EMAIL"});var d=this;User.findOne({email:a.body.email},function(b,f){b?c({status:403,datas:{message:b}}):null===f?c({status:403,message:"UNKNOW_USER_EMAIL"}):d.getValidationCode(f._id,function(b){200===b.status?Email_controller.lost_password(a,b,function(a){c({status:a.status,response_display:{message:"REQUEST_PASSWORD_MESSAGE",title:"REQUEST_PASSWORD_TITLE"},message:"PASSWORD_SENDED"})}):
c({status:b.status,datas:b})})})};module.exports.unregister=function(a,b,c){User.remove({_id:a.body.id},function(){c({status:200,message:"USER_DELETED",infos:a.body})});return a.body};
...
module.exports.updatePassword=function(a,b,c){var d=jwt.sign({secret:a.decoded.secret,email:a.decoded.email,password:a.password},config.secrets.global.secret,{expiresIn:"2 days"});User.update({email:a.email},{$set:{password:sha1(a.password),token:d,updated:Date.now()}},function(a,b){a?c({status:401,message:"CANT_UPDATE_PASSWORD",error:a,updated_token:d}):c({status:200,message:"PASSWORD_UPDATED",infos:b,updated_token:d})})};module.exports.check_user_session=function(a,b){};
module.exports.check_user=function(a,b){jwt.verify(a.options.user_token,config.secrets.global.secret,function(c,d){if(c)b({status:203,message:"UNAUTHORISED_TOKEN",response_display:{title:"Session invalide",message:"Vous n'\u00eates plus connect\u00e9.<br>veuillez vous reconnecter."},datas:c});else{a.decoded=d;if("undefined"===typeof d.password||"undefined"===typeof d.email)return b({status:203,message:"UNAUTHORISED",response_display:{title:"Session invalide",message:"Vous n'\u00eates plus connect\u00e9.<br>veuillez vous reconnecter."}}),
!1;User.find({email:d.email,password:sha1(d.password)},function(c,f){if(c)b({status:203,message:"UNAUTHORISED_TOKEN",datas:c,response_display:{title:"Session invalide",message:"Vous n'\u00eates plus connect\u00e9.<br>veuillez vous reconnecter."}});else if(0===f.length)b({status:203,message:"UNAUTHORISED",response_display:{title:"Session invalide",message:"Vous n'\u00eates plus connect\u00e9.<br>veuillez vous reconnecter."}});else{var e=jwt.sign({secret:f[0].user_secret,email:f[0].email,password:d.password},
