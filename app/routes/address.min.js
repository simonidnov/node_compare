...
address.get("/",function(a,b,d){Address_controller.get(a,b,function(c){console.log(c);b.redirect(307,"/account/addresses"+a.url.replace("/",""))})}).post("/",function(a,b,d){Address_controller.create(a,b,function(c){b.status(c.status).send(Auth_helper.addParams(c,a))})}).put("/",function(a,b,d){Address_controller.update(a,b,function(c){b.status(c.status).send(Auth_helper.addParams(c,a))})})["delete"]("/",function(a,b,d){Address_controller["delete"](a,function(c){b.status(c.status).send(Auth_helper.addParams(c,
a))})});module.exports=address;
