...
router.all("*",function(a,c,b){console.log("req.get('X-Forwarded-Proto') ",a.get("X-Forwarded-Proto"));console.log("req.get('host') ",a.get("host"));console.log("req.get('origin') ",a.get("origin"));console.log("req.get('X-Forwarded-Host') >>>>> ",a.get("X-Forwarded-Host"));var d=a.get("X-Forwarded-Proto").toLowerCase();"https"===d?(console.log("HAS HTTPS ",d),b()):"undefined"!==typeof a.get("X-Forwarded-Host")?-1===a.get("X-Forwarded-Host").indexOf("127.0.0.1")?c.redirect("https://"+a.get("X-Forwarded-Host")+
a.url):b():b()});router.use("/admin",admin);router.use("/admin/:any",admin);router.use("/users",users);router.use("/me",me);router.use("/account",account);router.use("/account/:any",account);router.use("/api",api);router.use("/media",media);router.use("/gmail",gmail);router.use("/address",address);router.use("/validation",validation);router.use("/templating",templating);router.use("/redirect",redirect);router.use("/auth",auth);router.use("/checking_session",checking_session);
