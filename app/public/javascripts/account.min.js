...
a.action&&index.sdk.api.put("/api/change_password/",{user_id:$("#security_form #user_id").val(),password:$("#security_form #change_password_old").val(),new_password:$("#security_form #change_password_new").val(),retype_new_password:$("#security_form #retype_change_password_new").val()},function(a){})})).init();this.address_form=new formular("#address_form",function(a){"validated"===a.status&&($("#addressLine1").val(),$("#postalCode").val(),$("#city").val(),$("#addressCountry").val());"hitted"===a.status&&
"submit"===a.action&&(a=account.address_form.get_datas(),index.sdk.api.post("/address/",a,function(a){window.location.reload()}))});this.address_form.init();this.private_form=(new formular("#private_datas",function(a){})).init();this.services_form=new formular("#services_form",function(a){"hitted"===a.status&&"submit"===a.action&&(a=account.services_form.get_datas(),index.sdk.api.put("/account/profile/",a,function(a){}))});this.services_form.init();0<$("#member_datas").length&&(this.member_form=(new formular("#member_datas",
function(a){if("undefined"!==typeof a.action)switch(a.action){case "save":var b={member_id:$("#member_datas").attr("data-memberid")};$.each($("#member_datas form").serializeArray(),function(a,d){b[d.name]=d.value});index.sdk.api.put("/me/members",b,function(a){window.location.reload()})}})).init());$(".delete_device").off("click").on("click",function(){var a=$(this);index.sdk.api.call("DELETE","/auth/device",{uid:$(this).attr("data-device_uid")},function(b){a.parent().parent().remove()})})},init_apple_pay:function(){document.getElementById("apple-pay-button").addEventListener("click",
