...
{};a[$(this).attr("data-type")]=$(this).attr("data-value");index.sdk.api.call("DELETE",$(this).attr("data-post"),a,function(a){console.log(a)});break;case "edit":0<$("#"+$(this).attr("data-edittemplate")).length&&$("#"+$(this).attr("data-edittemplate")).remove();$(".editable").removeClass("editable");var b=$(this).parent().parent();index.sdk.template($(this).attr("data-edittemplate"),{member_id:$(this).attr("data-value")},function(a){b.append(a);account.member_edit_form=(new formular("#member_edit_form",
...
$("#coupon_code").parent().removeClass("valid").addClass("invalid").append('<div class="input_error">'+$("#coupon_code").attr("data-errormessage")+"</div>")});if("hitted"===a.status&&"submit"===a.action){a=$("#order_transaction");var b=a.parents("form");a=$.extend({},a.data(),{token:function(a){b.append($("<input>").attr({type:"hidden",name:"stripeToken",value:a.id})).submit()}});StripeCheckout.open(a)}});this.checkout_form.init();this.public_form=new formular("#public_datas",function(a){"hitted"===
a.status&&"submit"===a.action&&(a=account.public_form.get_datas(),index.sdk.api.put("/account/profile/",a,function(a){}))});this.public_form.init();this.kid_form=(new formular("#add_kid",function(a){if("hitted"===a.status&&"submit"===a.action){var b={};$.each($("#add_kid form").serializeArray(),function(a,d){b[d.name]=d.value});index.sdk.api.post("/me/members",b,function(a){window.location.reload()})}})).init();this.security_form=(new formular("#security_form",function(a){"hitted"===a.status&&"submit"===
a.action&&index.sdk.api.put("/api/change_password/",{user_id:$("#security_form #user_id").val(),password:$("#security_form #change_password_old").val(),new_password:$("#security_form #change_password_new").val(),retype_new_password:$("#security_form #retype_change_password_new").val()},function(a){})})).init();this.address_form=(new formular("#address_form",function(a){"validated"===a.status&&($("#addressLine1").val(),$("#postalCode").val(),$("#city").val(),$("#addressCountry").val())})).init();this.private_form=
(new formular("#private_datas",function(a){})).init();this.services_form=new formular("#services_form",function(a){"hitted"===a.status&&"submit"===a.action&&(a=account.services_form.get_datas(),index.sdk.api.put("/account/profile/",a,function(a){}))});this.services_form.init();0<$("#member_datas").length&&(this.member_form=(new formular("#member_datas",function(a){if("undefined"!==typeof a.action)switch(a.action){case "save":var b={member_id:$("#member_datas").attr("data-memberid")};$.each($("#member_datas form").serializeArray(),
function(a,d){b[d.name]=d.value});index.sdk.api.put("/me/members",b,function(a){window.location.reload()})}})).init());$(".delete_device").off("click").on("click",function(){var a=$(this);index.sdk.api.call("DELETE","/auth/device",{uid:$(this).attr("data-device_uid")},function(b){a.parent().parent().remove()})})},init_apple_pay:function(){document.getElementById("apple-pay-button").addEventListener("click",function(a){a.preventDefault();Stripe.applePay.buildSession({requiredBillingContactFields:["postalAddress"],
requiredShippingContactFields:["phone"],countryCode:"FR",currencyCode:"EUR",total:{label:"joyvox.fr",amount:"4.99"}},function(a,c){console.log(a.token.card.address_line1);console.log(a.shippingContact.phoneNumber);c(!0)}).begin();return!1})}};
