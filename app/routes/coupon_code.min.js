...
coupon_code.get("/download/:offer",function(c,a,d){Auth_helper.validate_admin(c,function(b){200===b.status?d():a.status(403).send({title:"UNAUTHORIZED",message:"NO_ACCESS_RIGHTS_COUPONS"})})},function(c,a,d){Coupon_controller.getOffersCSV(c,a,function(b){var c=(new Json2csvParser({fields:["code","offer","amount"]})).parse(b.datas),d="./uploads/file"+Date.now()+".csv";fs.writeFile(d,c,function(e,f){e?a.status(200).send(e):(a.redirect(307,d.replace("./","/")),a.status(200).send({csv:c,datas:b.datas}),
a.end())})})});coupon_code.get("/offers",function(c,a,d){Auth_helper.validate_admin(c,function(b){200===b.status?d():a.redirect(307,"/auth?message=NO_ACCESS_RIGHTS_COUPONS")})},function(c,a,d){Coupon_controller.getOffers(c,a,function(b){a.status(b.status).send(b);a.end()})});coupon_code.post("/",function(c,a,d){a.status(200).send({message:"OUTCH PADAWAN ! YOU CANNOT CREATE ANY COUPON"});a.end()});
coupon_code.post("/create_offer",function(c,a,d){Auth_helper.validate_admin(c,function(b){200===b.status?d():a.redirect(307,"/auth?message=NO_ACCESS_RIGHTS_COUPONS")})},function(c,a,d){Coupon_controller.createOffer(c,a,function(b){a.status(b.status).send(b);a.end()})});coupon_code.put("/",function(c,a,d){Coupon_controller.update(c,a,function(b){a.status(b.status).send(b);a.end()})});
coupon_code["delete"]("/",function(c,a,d){Auth_helper.validate_admin(c,function(b){200===b.status?d():a.redirect(307,"/auth?message=NO_ACCESS_RIGHTS_COUPONS")})},function(c,a,d){Coupon_controller["delete"](c,a,function(b){a.status(b.status).send(b);a.end()})});module.exports=coupon_code;
