...
module.exports.reset_session=function(a,b,c){User.findOne({_id:b},function(d,f){d?c({status:401,code:d.code,error:d,message:d.message}):(user_infos=JSON.parse(JSON.stringify(f)),user_infos.current_device=device_uid,Members_model.get(b,null,function(d){user_infos.members=d.datas;Address_model.get(b,null,function(b){user_infos.address=b.datas;a.session.Auth=user_infos;c({status:200,message:"SESSION_UPDATED",datas:user_infos})})}))})};
...
module.exports.validAccount=function(a,b,c){jwt.verify(b.validation_code,config.secrets.global.secret,function(a,b){a?console.log("ERROR JWT :::: ",a):console.log("DECODED ",b)});User.findOne({email:b.email,validation_code:b.validation_code},function(b,f){b||null===f?c({status:304,message:"UNAUTHORISED_TOKEN",datas:b}):User.update({_id:f._id},{$set:{validated:!0,certified:!0}},function(b,d){b?c({status:204,message:"UNAUTHORISED_TOKEN",datas:b}):self.reset_session(a,f._id,function(a){c({status:200,
message:"USER_VALIDATED",datas:d})})})})};module.exports.getUsersDevice=function(a,b){User.find({devices:{$elemMatch:{uid:a}}},{email:1,avatar:1,pseudo:1},function(a,d){a||0===d.length?b({status:304,message:"NEW_DEVICE",datas:a}):b({status:200,message:"liste des utilisateurs ayant utilis\u00e9 ce device",users_device:d})})};
